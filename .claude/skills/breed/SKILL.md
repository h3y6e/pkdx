...
`ウォッシュロトム` / `キュウコン（アローラ）` 等のフォーム違いも入力名をそのまま渡せば引ける。

**結果が空の場合**:

- フォーム違いの可能性: 原種名で `pkdx query` を実行し、返り値の `forms[]` (その version で実在するフォームの一意名配列、例 `["キュウコン（アローラ）"]` / `["ウォッシュロトム", ...]`) からユーザー意図に合う名前を選んで再 query
